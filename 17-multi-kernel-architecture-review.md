...
`date`: 2026-09-17  
`status`: **architecture-review-draft** — 对多核方向进行理论审阅并给出推荐收敛方案；不表示已经定案，不授权实施。
...
[16-self-hosting-architecture-review.md](./16-self-hosting-architecture-review.md) 建议首版单 GenerationManager、stable/candidate/previous slot。多 Kernel 方向不推翻它，而是把 slot 变成可运行的角色：
