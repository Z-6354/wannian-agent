...
```text
0.2.1  Agent Loop + 错误码 + Turn 接线 + live/execute   （别名 K01）
       施工单 → plans/k01-agent-loop.md
   ↓
0.2.2  ToolRuntime + ToolProfile（本节点可安全工具）   （别名 K02）
0.2.3  Memory + Relationship（成长核心 · 高优先级）   （别名 K03）
       边界 → research/memory.md
0.2.4  Outbox / SSE（分享时才投用户）                 （别名 K04）
