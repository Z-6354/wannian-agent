# 讨论与研究稿

`status`: **reference** — 不是施工单。  
版本：[产品概览 §3](../product/01-overview.md)。进度：[实施清单](../guide/01-checklist.md)。排期：[roadmap](../plans/roadmap.md)。

## 维护规则

| 要改什么 | 只改 |
|----------|------|
| 外部循环怎么切 | [agent-loop-survey.md](./agent-loop-survey.md) |
| 十方工具模块与去重目录 | [ten-agent-tools.md](./ten-agent-tools.md) |
| 本项目模块 | [wannian-loop-modules.md](./wannian-loop-modules.md) |
| 世界 / 工具 / Jev | [world-evolution-and-extensions.md](./world-evolution-and-extensions.md) |
| 设备 / 节点 / 角色（v0.3） | [multi-node-companion-world.md](./multi-node-companion-world.md) |
| 全档排期 | [roadmap.md](../plans/roadmap.md) |
| **0.2.1** 步骤与验收命令 | [k01-agent-loop.md](../plans/k01-agent-loop.md) |
| 执行中插话（Cancel/Queue/Steer）· **专题** | [in-flight-user-message.md](./in-flight-user-message.md) |
| 微信通道 & 世界树/用户时序冲突 | [wechat-and-world-timing.md](./wechat-and-world-timing.md) |
| 记忆已确认边界 | [memory.md](./memory.md) |
| 记忆注入超限他者对照 | [memory-inject-budget-peers.md](./memory-inject-budget-peers.md) |
| Agent 运行日志同业对照（0.2.3-L） | [agent-run-logging-peers.md](./agent-run-logging-peers.md) |

## 现行（按读序）

| | 文档 | 档 |
|--|------|----|
| 1 | [agent-loop-survey.md](./agent-loop-survey.md) | 对照 |
| 1b | [ten-agent-tools.md](./ten-agent-tools.md) | 工具模块 / 去重目录 |
| 2 | [wannian-loop-modules.md](./wannian-loop-modules.md) | v0.2 模块 |
| 3 | [in-flight-user-message.md](./in-flight-user-message.md) | 插话时序 |
| 4 | [wechat-and-world-timing.md](./wechat-and-world-timing.md) | 微信 & 世界冲突 |
| 5 | [world-evolution-and-extensions.md](./world-evolution-and-extensions.md) | 叙事；世界树属 v0.3 |
| 6 | [multi-node-companion-world.md](./multi-node-companion-world.md) | **v0.3** |

## 记忆

- 已确认产品边界：[memory.md](./memory.md)。未决从该文 §6 继续。
- **0.2.3 记忆（已交付）：** [memory-system-0.2.3.md](./memory-system-0.2.3.md) §4 + §4.1 · [k03-memory.md](../plans/k03-memory.md)
- 旧「仅正交轴」稿指针：[memory-axes-0.2.3.md](./memory-axes-0.2.3.md)（已迁移，勿单独施工）

## 历史（更后，不是产品 v0.2）

`history/v02-*` 谈自进化、宿主主备、旧多设备组合。**产品 v0.2 = 单核 harness。** 索引见 [history/README.md](./history/README.md)。
