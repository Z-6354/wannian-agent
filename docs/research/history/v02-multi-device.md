...
`date`: 2026-09-17  
`status`: **superseded** — 历史讨论。现行拓扑是 [设备 / 节点 / 角色](./multi-node-companion-world.md)，不要按本文的三核固定槽施工。  
`status-original`: brainstorm-draft — 讨论单体独立运行、多个节点组合及多设备部署；不表示方案已确认，不授权实施。
